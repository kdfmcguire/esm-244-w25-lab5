...
ggplot(seeds_df_long, aes(x=value)) +
  geom_histogram(aes(fill = variety)) +
  theme_light() +
...
```{r}
#| fig-cap: Based on the graph, we will choose to use 3 groups to initiate the k-means cluster. The bend in the plot is most pronounced here - after 3 groups, the total within sum of squares does not decrease much with each added group.
...
```{r}
seeds_cl <- data.frame(seeds_complete, cluster_no = factor(seeds_km$cluster))
```
...
ggplot(seeds_cl, aes(x=a, y=asym)) +
  geom_point(aes(color = cluster_no, shape = variety)) +
  theme_light() +
  labs(x = "Area", y = "Asymmetry")
...
```{r}
#| tbl-cap: Contingency table for the kmeans cluster groupings and varieties.
...
1. choose top 20 emitters 
2. drop NAs
3. select only numeric cols
4. scale data
...
```{r}
#| fig-cap: A. Complete Linkage ; B. Single Linkage
p_complete + p_single + plot_annotation(tag_levels = 'A')
```
